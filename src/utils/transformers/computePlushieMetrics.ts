import type { PlushieRecord, Metrics, DayCell } from "@pages/Plushies/Plushies.types";

const GOAL = 5;
const MAIN_TARGET = 3000;
const STRETCH_TARGET = 3500;
...
export function dateKeyOf(d: Date) {
  ...
  export function computePlushiesMetrics(
    ...
    const baseline = Math.max(0, Math.floor(opts?.baseline ?? 0));
    const includeBaselineInAhead = !!opts?.includeBaselineInAhead;
  
    const today = todayKey ?? dateKeyOf(new Date());
  
    // Build countsByDate and find earliest date present
    ...
    for (const dk of allDays) {
      ...
      days.push({ dateKey: dk, raw, effective, complete, bankAfter: bank });
    }
    ...
    const aheadDrops = (includeBaselineInAhead ? total : rawTotal) - expectedToDate;
