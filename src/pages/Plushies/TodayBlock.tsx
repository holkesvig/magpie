 ...
 * - initialNotes?: if you also track notes per-day and have today's notes handy, pass them here (optional)
 */
export default function TodayBlock({ metrics, initialNotes }: { metrics: Metrics; initialNotes?: string }) {
  ...
  const prevCell = metrics.days.length > 1 ? metrics.days[metrics.days.length - 2] : undefined;
  ...
  const handleSaveManual = () => {
    const val = typeof manualCount === "number" ? Math.max(0, Math.floor(manualCount)) : null;
    if (val == null) return;
    ...
    <section style={{ display: 'flex', flexDirection: 'column', gap: '20px', width: '100vw', padding: '5%' }}>
      ...
      <div className="grid gap-3 md:grid-cols-3">
        ...
        <div className="rounded-xl border p-3">
          ...
          {coveredByBank > 0 && (
            <div className="mt-1 text-xs text-gray-500">+{coveredByBank} from bank</div>
          )}
        </div>
        ...
        <div className="rounded-xl border p-3">
          ...
          <div className="mt-1 text-xs text-gray-500">
            = {Math.floor(bankBefore / goal)} {Math.floor(bankBefore / goal) === 1 ? "day" : "days"}
          </div>
        </div>
      </div>
      ...
      {editable ? (
        <div className="grid gap-3 md:grid-cols-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <input
              ...
              inputMode="numeric"
              value={manualCount}
              onChange={(e) => setManualCount(e.target.value === "" ? "" : Number(e.target.value))}
        ...
        <div className="text-xs text-gray-500">View-only. Unlock editing to change today’s count.</div>
      )}
    </section>
  );
}
