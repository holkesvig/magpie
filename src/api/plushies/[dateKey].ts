...
const AUTH_HEADER = { Authorization: `Bearer ${process.env.AIRTABLE_API_KEY}`, 'Content-Type': 'application/json' };
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (!process.env.EDIT_KEY || editKey !== process.env.EDIT_KEY) {
  ...
  try {
    // 1) Find existing record by dateKey
    const searchParams = new URLSearchParams();
    searchParams.set('filterByFormula', `{dateKey} = "${dateKey}"`);
    searchParams.set('pageSize', '1');

    const findResp = await fetch(`${AIRTABLE_API}?${searchParams.toString()}`, {
    ...
    // 3) Create or Update
    if (!existing) {
      // Create with date set to dateKey (date-only field will coerce)
      const createBody = {
        records: [
          {
            fields: {
